...
and string_of_expr = function
 ...
 | Call(s, el) -> string_of_id s ^ string_of_list string_of_expr el "(" ", " ")" true
...
let string_of_glob = function
  | ConstDecl(bind, expr) -> string_of_bind bind ^ " " ^ string_of_expr expr ^ ";"
...
and string_of_program (ns_list, glob_list, fn_list) =
  String.concat "\n" (List.map string_of_ns ns_list) ^ "\n" ^
  String.concat "\n" (List.map string_of_glob glob_list) ^ "\n" ^
